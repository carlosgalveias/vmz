...
describe('modules', () => {
	...
	it('process events', () => {
		...
		const sandbox = vm.run("global.vmz_HANDLER = function() { vmz_COUNTER++ }; process.on('exit', vmz_HANDLER); module.exports = global;");
		process.emit('exit');
		assert.strictEqual(sandbox.vmz_COUNTER, 1);
		assert.strictEqual(vm.run("module.exports = process.listeners('exit')[0] === vmz_HANDLER;"), true);
		vm.run("process.removeListener('exit', vmz_HANDLER);");
		process.emit('exit');
		assert.strictEqual(sandbox.vmz_COUNTER, 1);
	...
	it('missing contextify attack', () => {
		...
		// https://github.com/patriksimek/vmz/issues/276
		...
		// https://github.com/patriksimek/vmz/issues/285
