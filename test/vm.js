...
describe('contextify', () => {
	...
	it('symbol', () => {
		assert.strictEqual(vm.run("Symbol.for('foo') === test.symbol2"), false); // its false as we are suing a mocked symbol internally 
		...
		// assert.strictEqual(vm.run('Symbol.keyFor(test.symbol2)'), 'foo'); // internally we dont have a gloval view of everey symbol as we are mocking internally
	});
...
describe('VM', () => {
	...
	if (NODE_VERSION >= 10) {
		it('eval/wasm', () => {
			...
			const vmz = new VM({eval: false});
			assert.throws(() => vmz.run('eval("1")'), /Code generation from strings disallowed for this context/);
		});
	}
	...
	if (NODE_VERSION > 7) {
		...
		it('async', () => {
			const vmz = new VM({fixAsync: true});
			assert.throws(() => vmz.run('(async function(){})'), /Async not available/, '#1');
			assert.strictEqual(vmz.run('Object.getPrototypeOf((function*(){}).constructor)'), vmz.run('Function'), '#2');
			assert.throws(() => vmz.run('new Function("(as"+"ync function(){})")'), /Async not available/, '#3');
			assert.throws(() => vmz.run('new (function*(){}).constructor("(as"+"ync function(){})")'), /Async not available/, '#4');
			assert.throws(() => vmz.run('Promise.resolve().then(function(){})'), /Async not available/, '#5');
			if (Promise.prototype.finally) assert.throws(() => vmz.run('Promise.resolve().finally(function(){})'), /Async not available/, '#6');
			if (Promise.prototype.catch) assert.throws(() => vmz.run('Promise.resolve().catch(function(){})'), /Async not available/, '#7');
			assert.throws(() => vmz.run('eval("(as"+"ync function(){})")'), /Async not available/, '#8');
			assert.throws(() => vmz.run('Function')('(async function(){})'), /Async not available/, '#9');
			assert.doesNotThrow(() => vmz.run(`
				let a = {import: 1}
	...
	it('proxy trap errors', () => {
		const vmz = new VM();
		assert.doesNotThrow(() => {
			Reflect.ownKeys(vmz.run('(function(){}).bind(null)'));
		});
	});
	...
	it('frozen unconfigurable access', () => {
		...
		assert.doesNotThrow(()=>{
			vmz.run('x => x.prop')(Object.freeze({prop: {}}));
		});

		assert.doesNotThrow(()=>{
			vmz.run('x => Object.getOwnPropertyDescriptor(x, "prop")')(Object.freeze({prop: {}}));
		});

		assert.doesNotThrow(()=>{
			vmz.run('x => x.prop')(Object.defineProperty({}, 'prop', {value: {}}));
		});

		assert.doesNotThrow(()=>{
			vmz.run('x => Object.isExtensible(x)')(Object.freeze({prop: {}}));
		});

		assert.doesNotThrow(()=>{
			vmz.run('x => {Object.preventExtensions(x); Object.getOwnPropertyDescriptor(x, "prop")}')({prop: {}});
		});

		assert.strictEqual(vmz.run('x => {Object.preventExtensions(x); return Object.getOwnPropertyDescriptor(x, "prop").value}')({prop: obj}), obj);

	});

	it('various attacks #1', () => {
		const vmz = new VM({sandbox: {log: console.log, boom: () => {
			throw new Error();
		}}});

		assert.strictEqual(vmz.run("this.constructor.constructor('return Function(\\'return Function\\')')()() === this.constructor.constructor('return Function')()"), true);

		assert.throws(() => vmz.run(`
	...
	it('internal state attack', () => {
		const vmz = new VM();
		assert.throws(() => vmz.run(`${INTERNAL_STATE_NAME}="async";`), /Use of internal vmz state variable/);
		assert.throws(() => vmz.run(`const ${INTERNAL_STATE_NAME} = "async";`), /Use of internal vmz state variable/);
		assert.throws(() => vmz.run(`var ${INTERNAL_STATE_NAME} = "async";`), /Use of internal vmz state variable/);
		assert.throws(() => vmz.run(`let ${INTERNAL_STATE_NAME} = "async";`), /Use of internal vmz state variable/);
		assert.throws(() => vmz.run(`class ${INTERNAL_STATE_NAME} {}; // async`), /Use of internal vmz state variable/);
		assert.throws(() => vmz.run(`function ${INTERNAL_STATE_NAME} () {}; // async`), /Use of internal vmz state variable/);
	});
	...
	it('instanceof attack', () => {
		// https://github.com/patriksimek/vmz/issues/174
	...
	it('__defineGetter__ / __defineSetter__ attack', () => {
		// https://github.com/patriksimek/vmz/issues/176
	...
	it('__lookupGetter__ / __lookupSetter__ attack', () => {
		// https://github.com/patriksimek/vmz/issues/184
	...
	it('contextifying a contextified value attack', () => {
		// https://github.com/patriksimek/vmz/issues/175
		// https://github.com/patriksimek/vmz/issues/177
		// https://github.com/patriksimek/vmz/issues/186
	...
	it('proxy trap via Object.prototype attack', () => {
		// https://github.com/patriksimek/vmz/issues/178
	...
	it('function returned from construct attack', () => {
		// https://github.com/patriksimek/vmz/issues/179
	...
	it('throw while accessing propertyDescriptor properties', () => {
		// https://github.com/patriksimek/vmz/issues/178#issuecomment-450904979

		const vmz = new VM();

		assert.strictEqual(vmz.run(`(function(){
			var process;
	...
	it('Symbol.hasInstance attack', () => {
		// https://github.com/patriksimek/vmz/issues/178#issuecomment-450978210
	...
	*/
	...
	it('Error.prepareStackTrace attack', () => {
		const vmz = new VM();
		const sst = vmz.run('Error.prepareStackTrace = (e,sst)=>sst;const sst = new Error().stack;Error.prepareStackTrace = undefined;sst');
		assert.strictEqual(vmz.run('sst=>Object.getPrototypeOf(sst)')(sst), vmz.run('Array.prototype'));
		assert.throws(()=>vmz.run('sst=>sst[0].getThis().constructor.constructor')(sst), /TypeError: Cannot read propert.*constructor/);
		assert.throws(()=>vmz.run(`
